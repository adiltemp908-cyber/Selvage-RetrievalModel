...
- **Part E** — `test_report.md`, generated by `generate_report.py` running
  the full a predetermined query set (10 free-text, 5 phrase, 3 proximity at
  different k, 1 unknown-term query) against the real index, with no
  document IDs hard-coded, plus a written comparison of VSM vs. exact-phrase
  result sets for each phrase query
